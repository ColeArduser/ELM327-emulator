...
def get_custom_values():
    ...
    speed_start_values = [0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 60]
    rpm_start_values = [1000, 2000, 2250, 2500, 2750, 3000, 3250, 3500, 3500, 3750, 4000, 4000, 4250, 2000]

    speed_end_values = [60, 60, 55, 50, 45, 40, 35, 30, 25, 20, 15, 10, 5, 0]
    rpm_end_values = [2000, 2000, 2000, 2000, 2000, 2000, 2000, 2000, 2000, 2000, 2000, 2000, 2000, 1000]

    speed_values += speed_start_values
    ...
    speed_values += speed_end_values
    rpm_values += rpm_end_values
    num_seconds = num_seconds + len(speed_start_values) + len(speed_end_values)
    return speed_values, rpm_values, num_seconds


def get_coolant_values(num_seconds):
    coolant_values = []
    current_coolant = 60

    for i in range(0, 5 * 20):
        coolant_values += [current_coolant, current_coolant, current_coolant]
        current_coolant += 1
    num_seconds = num_seconds - len(coolant_values)

    for i in range(0, 4 * 12):
        if num_seconds <= 0:
            return coolant_values
        elif num_seconds <= 5:
            coolant_values += [current_coolant] * num_seconds
            current_coolant += 1
            return coolant_values
        else:
            coolant_values += [current_coolant] * 5
            current_coolant += 1
            num_seconds -= 5

    if num_seconds > 0:
        for i in range(0, num_seconds):
            new_coolant_temp = current_coolant + random.choice(range(-1, 2, 1))
            if new_coolant_temp < 190:
                new_coolant_temp = 190
            elif new_coolant_temp > 220:
                new_coolant_temp = 220
            coolant_values.append(new_coolant_temp)
            current_coolant = new_coolant_temp
    return coolant_values


def get_oil_values(num_seconds):
    oil_values = []
    current_oil = 60

    for i in range(0, 6 * 20):
        oil_values += [current_oil, current_oil, current_oil]
        current_oil += 1
    num_seconds = num_seconds - len(oil_values)

    for i in range(0, 4 * 15):
        if num_seconds <= 0:
            return oil_values
        elif num_seconds <= 4:
            oil_values += [current_oil] * num_seconds
            current_oil += 1
            return oil_values
        else:
            oil_values += [current_oil] * 4
            current_oil += 1
            num_seconds -= 4

    if num_seconds > 0:
        for i in range(0, num_seconds):
            new_oil_temp = current_oil + random.choice(range(-1, 2, 1))
            if new_oil_temp < 230:
                new_oil_temp = 230
            elif new_oil_temp > 260:
                new_oil_temp = 260
            oil_values.append(new_oil_temp)
            current_oil = new_oil_temp
    return oil_values


def mph_to_kph(mph):
    rounded = round(mph * 1.60934)
    return_string = f'{rounded:x}'
    if len(return_string) % 2 != 0:
        return_string = '0' + return_string
    return return_string


def construct_rpm(rpm_value):
    return_value = f'{rpm_value * 4:04X}'
    return return_value


def f_to_c(f_value):
    return round((f_value - 32) * 5 / 9) + 40
...
ObdMessage = {
            ...
            'ResponseFooter': \
                lambda self, cmd, pid, uc_val: (
                        ...
                        + ' ' + HD(ECU_R_ADDR_H) + SZ('04') + DT('41 0C '
                                                                 + self.sequence(pid, base=2400, max=200, factor=80,
                                                                                 n_bytes=2))
                )
        },
        'SPEED': {
            ...
            'ResponseFooter': \
                lambda self, cmd, pid, uc_val: (
                        ...
                        + ' ' + HD(ECU_R_ADDR_H) + SZ('03') + DT('41 0D '
                                                                 + self.sequence(pid, base=0, max=30, factor=4,
                                                                                 n_bytes=1))
                )
        },
        'INTAKE_TEMP': {
    ...
    'custom': {
        ...
        'COOLANT_TEMP': {
            ...
            'Header': ECU_ADDR_E,
            'Response': [
                HD(ECU_R_ADDR_E) + SZ('03') + DT('41 05 ' + int_to_hex(f_to_c(coolant))) for coolant in
                CUSTOM_COOLANT_VALUES
            ]
        },
        'SHORT_FUEL_TRIM_1': {
        ...
        'RPM': {
            ...
            'Header': ECU_ADDR_E,
            'Response': [
                HD(ECU_R_ADDR_E) + SZ('04') + DT('41 0C ' + construct_rpm(rpm)) for rpm in CUSTOM_RPM_VALUES
            ]
        },
        'SPEED': {
            'Request': '^010D' + ELM_FOOTER,
            ...
            'Header': ECU_ADDR_E,
            'Response': [
                HD(ECU_R_ADDR_E) + SZ('03') + DT(f"41 0D " + mph_to_kph(speed)) for speed in CUSTOM_SPEED_VALUES
            ]
        },
        'TIMING_ADVANCE': {
        ...
        'RUN_TIME': {
            ...
            'Header': ECU_ADDR_E,
            'Response': [
                HD(ECU_R_ADDR_E) + SZ('04') + DT('41 1F ' + int_to_hex_4(seconds)) for seconds in
                list(range(1, CUSTOM_NUMBER_OF_SECONDS + 1)) + [0]
            ]
        },
        'PIDS_B': {
            'Request': '^0120' + ELM_FOOTER,
        ...
        'PIDS_C': {
            ...
            'Header': ECU_ADDR_E,
            'Response': [
                HD(ECU_R_ADDR_E) + SZ('06') + DT('41 40 7A 1C 80 10')
            ]
        },
        'CONTROL_MODULE_VOLTAGE': {
            'Request': '^0142' + ELM_FOOTER,
        ...
        'OIL_TEMP': {
            ...
            'Header': ECU_ADDR_E,
            'Response': [
                HD(ECU_R_ADDR_E) + SZ('03') + DT('41 5C ' + int_to_hex(f_to_c(oil))) for oil in CUSTOM_OIL_VALUES
            ]
        },
        'PIDS_D': {
